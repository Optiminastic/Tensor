...
function ModelMesh({
  ...
  // Reported through refs so a changing callback identity never re-runs the
  // effects below. onBounds is an inline arrow from the parent (new identity
  // every render); depending on it directly makes the effect fire each render,
  // and since it calls setState in the parent, that spins an infinite
  // re-render loop that jitters the whole canvas. Mirrors onTiming/onColours.
  ...
  useEffect(() => onMeasureRef.current(measure), [measure])
  useEffect(() => onBoundsRef.current?.(boxMin, boxMax), [boxMin, boxMax])
