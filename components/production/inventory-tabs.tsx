...
interface InventoryTabsProps {
  ...
  unmappedColours: UnmappedColour[]
}
...
export function InventoryTabs({
  ...
  const tabs: TabItem[] = [
    ...
    // Counted by unmapped spools rather than by entries: the number that
    // matters is the work left, not the work done, and a colour nobody has
    // named is a bed that cannot be matched to a machine.
    { value: COLOURS_TAB, label: 'Colour map', count: unmappedColours.length },
  ]
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {tab === COLOURS_TAB ? (
        <ColourMapView brand={brand} entries={colourMap} unmapped={unmappedColours} />
      ) : null}
    </div>
  )
}
